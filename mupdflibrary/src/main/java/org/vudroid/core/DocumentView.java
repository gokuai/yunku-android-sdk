...
public class DocumentView extends View implements ZoomListener {
    ...
    public DocumentView(Context context, CurrentPageModel currentPageModel) {
        super(context);
    ...
    private void init() {
        ...
        for (int i = 0; i < decodeService.getPageCount(); i++) {
            ...
            int width = decodeService.getEffectivePagesWidth(i);
            int height = decodeService.getEffectivePagesHeight(i);
    ...
    private void currentPageChanged() {
        post(new Runnable() {
            public void run() {
                //FIXME
                mCurrentPageModel.setCurrentPageIndex(getCurrentPage(), pages.size());
            }
        });
    }
    ...
    public void showDocument(ExceptionCatcherRunnable.ExceptionListener listener){
        // use post to ensure that document view has width and height before decoding begin

        post(new ExceptionCatcherRunnable(listener,new Runnable()  {
            public void run() {
                init();
                updatePageVisibility();
            }
...
//        });
    ...
    class MySimpleOnGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            mCurrentPageModel.dispatch(new SingleTapListener.SingleTapEvent());
            return true;
        }
