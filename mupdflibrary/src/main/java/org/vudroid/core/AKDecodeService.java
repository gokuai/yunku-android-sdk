...
public class AKDecodeService implements DecodeService {
    ...
    public void open(Context context, Uri fileUri) {
        document = codecContext.openDocument(context, PathFromUri.retrieve(contentResolver, fileUri));
    }
    ...
    public int getEffectivePagesWidth(int pageIndex) {
        final CodecPage page = getPage(pageIndex);
        return getScaledWidth(page, calculateScale(page));
    }

    public int getEffectivePagesHeight(int pageIndex) {
        final CodecPage page = getPage(pageIndex);
