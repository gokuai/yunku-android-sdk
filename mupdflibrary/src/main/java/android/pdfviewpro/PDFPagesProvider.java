...
import android.lib.pagesview.OnImageRenderedListener;
import android.lib.pagesview.PagesProvider;
import android.lib.pagesview.RenderingException;
import android.lib.pagesview.Tile;
...
public class PDFPagesProvider extends PagesProvider {
	...
	private static class BitmapCache {
		...
		Bitmap get(Tile k) {
			...
			if ((this.hits + this.misses) % 100 == 0 && (this.hits > 0 || this.misses > 0)) {
				Log.d("android.pdfviewpro.pagecache", "hits: " + hits + ", misses: " + misses + ", hit ratio: " + (float)(hits) / (float)(hits+misses) +
	...
	public PDFPagesProvider(Activity activity, MuPDFCore pdf, boolean skipImages,
	...
	private Bitmap renderBitmap(Tile tile) throws RenderingException {
		synchronized(tile) {
            ...
            /*System.out.println(String.format("renderBitmap zoom:%d, pageW-:%d, pageH-:%d, patchX:%d, patchY:%d, patchW:%d, patchH:%d",
                tile.getZoom(),
